...
def predict_path_safety(path: List[Tuple[float, float]], names: List[str] | None = None) -> Dict[str, Any]:
    ...
    for (lat, lon), area_name in zip(path, pts_names):
        ...
        segment_infos.append({"point": (lat, lon), "score": info["final_score"], "breakdown": info["breakdown"], "raw": info["raw"]})
    ...
    return {"segment_scores": segment_infos, "path_score": round(path_score, 1), "band": path_band, "recommendation": recommendation}


def pretty_print_result(area: str, result: Dict[str, Any]):
    print(f"\n--- Safety Score for {area} ---")
    print(f"Final score: {result['final_score']}/100  ({result['band']})")
    bd = result["breakdown"]
    print(f"Breakdown: Weather {bd['weather']}/100, Incident {bd['incident']}/100, Crowd {bd['crowd']}/100, News {bd['sentiment']}/100, Road {bd['road']}/100")
    print("Raw signals:")
    print(json.dumps(result["raw"], indent=2))

if __name__ == "__main__":
    area_ex = input("Enter a location name: ").strip()
    coords = get_lat_lon_from_name(area_ex)
    if coords:
        lat_ex, lon_ex = coords
        print(f"Computing safety score for {area_ex} ({lat_ex}, {lon_ex}) ...")
